...
const props = defineProps({
    ...
    assignmentYear: { type: Number, default: null },
    assignmentYears: { type: Array, default: () => [] },
    evaluatorAssignments: { type: Array, default: () => [] },
});

const months = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'];
const periodLabel = computed(() => `${months[(props.period?.bulan || 1) - 1]} ${props.period?.tahun || ''}`);
const inputPeriodLabel = computed(() => { const date = new Date(props.period.tahun, props.period.bulan, 1); return `Target selesai 5 ${months[date.getMonth()]} ${date.getFullYear()}`; });
...
const assignmentModal = ref(null);
const assignForm = useForm({ year: props.assignmentYear || props.period?.tahun || '', month: '', evaluator_id: '' });
const assignmentRow = computed(() => props.evaluatorAssignments.find((row) => row.month === Number(assignForm.month)) || null);
const eligibleEvaluatorsForMonth = (month) => props.eligibleEvaluators.filter((evaluator) => !evaluator.eligible_months?.length || evaluator.eligible_months.includes(month));
const openAssignment = (row) => {
    if (row.locked) return;
    assignForm.year = props.assignmentYear || props.period?.tahun || '';
    assignForm.month = row.month;
    assignForm.evaluator_id = row.evaluator_id || '';
    assignmentModal.value = row;
};
const closeAssignment = () => { if (!assignForm.processing) assignmentModal.value = null; };
const submitAssign = () => assignForm.post(route('dashboard.kpi.mpa.assign'), { preserveScroll: true, onSuccess: () => { assignmentModal.value = null; } });
const changeAssignmentYear = (year) => router.get(route('dashboard.kpi.mpa', props.period.id), { assignment_year: year }, { preserveScroll: true, preserveState: false });
const form = useForm({
    ...
    <InternalDashboardLayout title="MPA" :user="user" content-width="wide">
        <div class="w-full space-y-5 p-6">
            ...
            <template v-if="!isFormView">
                <section v-if="isSuperAdmin" class="rounded-xl border border-blue-100 bg-white p-5 shadow-sm"><div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><h2 class="text-lg font-bold text-[#0b347d]">👥 Penetapan Penilai MPA</h2><p class="text-xs text-[#53709d]">Tetapkan satu penilai untuk setiap bulan. Pengaturan bulan mendatang dapat disiapkan lebih awal.</p></div><label class="flex items-center gap-2 text-xs font-semibold text-[#53709d]">Tahun<select :value="assignmentYear" @change="changeAssignmentYear(Number($event.target.value))" class="rounded-lg border border-blue-200 bg-white px-3 py-2 text-xs font-semibold text-[#173f82]"><option v-for="year in assignmentYears" :key="year" :value="year">{{ year }}</option></select></label></div><div class="mt-4 overflow-x-auto rounded-lg border border-blue-100"><table class="w-full min-w-[680px] text-xs"><thead class="bg-[#f3f7fd] text-left font-bold text-[#53709d]"><tr><th class="px-3 py-2.5">Bulan</th><th class="px-3 py-2.5">Penilai</th><th class="px-3 py-2.5">Status</th><th class="px-3 py-2.5 text-right">Aksi</th></tr></thead><tbody><tr v-for="row in evaluatorAssignments" :key="row.month" class="border-t border-blue-50"><td class="px-3 py-2.5 font-semibold text-[#173f82]">{{ months[row.month - 1] }} {{ assignmentYear }}</td><td class="px-3 py-2.5 text-[#53709d]"><span v-if="row.evaluator_name" class="font-semibold text-[#173f82]">{{ row.evaluator_name }}<small v-if="row.evaluator_position" class="ml-1 text-[#6480aa]">({{ row.evaluator_position }})</small></span><span v-else class="text-slate-400">Belum Ditentukan</span><span v-if="row.eligibility_warning" class="ml-2 rounded bg-amber-50 px-2 py-1 text-[10px] font-semibold text-amber-700">Penilai tidak lagi memenuhi syarat</span></td><td class="px-3 py-2.5"><span class="rounded-full px-2.5 py-1 text-[10px] font-bold" :class="row.status === 'locked' ? 'bg-slate-100 text-slate-600' : row.status === 'assigned' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'">{{ row.status === 'locked' ? 'Terkunci' : row.status === 'assigned' ? 'Ditetapkan' : 'Belum Ditentukan' }}</span></td><td class="px-3 py-2.5 text-right"><button v-if="!row.locked" type="button" @click="openAssignment(row)" class="rounded-lg border border-blue-200 px-3 py-1.5 text-[11px] font-bold text-blue-700 hover:bg-blue-50">{{ row.evaluator_id ? 'Edit' : 'Atur' }}</button><span v-else class="text-[11px] font-semibold text-slate-400">🔒 Terkunci</span></td></tr></tbody></table></div></section>
        ...
        <div v-if="assignmentModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4" @click.self="closeAssignment"><div class="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl"><div class="flex items-start justify-between gap-3"><div><h3 class="text-lg font-bold text-[#0b347d]">Penetapan Penilai MPA</h3><p class="mt-1 text-xs text-[#53709d]">{{ months[assignmentModal.month - 1] }} {{ assignmentYear }}</p></div><button type="button" class="text-xl text-slate-400" @click="closeAssignment">×</button></div><label class="mt-5 block text-xs font-semibold text-[#53709d]">Penilai</label><select v-model="assignForm.evaluator_id" class="mt-2 w-full rounded-lg border border-blue-200 bg-white px-3 py-2 text-sm text-[#173f82]"><option value="" disabled>Pilih Penilai</option><option v-for="ev in eligibleEvaluatorsForMonth(Number(assignForm.month))" :key="ev.id" :value="ev.id">{{ ev.position ? ev.name + ' (' + ev.position + ')' : ev.name }}</option></select><p v-if="assignForm.errors.evaluator_id" class="mt-2 text-xs font-semibold text-rose-600">{{ assignForm.errors.evaluator_id }}</p><div class="mt-5 flex justify-end gap-2"><button type="button" @click="closeAssignment" class="rounded-lg border border-slate-200 px-4 py-2 text-xs font-bold text-slate-600">Batal</button><button type="button" @click="submitAssign" :disabled="!assignForm.evaluator_id || assignForm.processing" class="rounded-lg bg-blue-600 px-4 py-2 text-xs font-bold text-white disabled:opacity-50">Simpan</button></div></div></div>        </div>
