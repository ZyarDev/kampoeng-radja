...
const props = defineProps({
    ...
    pendingApprovals: Array, dailyCalendar: Array, calendarMonth: Number, calendarYear: Number, employeePeriods: { type: Array, default: () => [] },
});
...
const statusLabel = computed(() => props.report.status === 'approved' ? (props.approvalInfo?.source === 'super_admin_takeover' ? 'Diambil Alih Super Admin' : 'Disetujui Atasan') : props.report.status === 'waiting_approval' ? 'Menunggu Persetujuan' : props.isEligible ? 'Dalam Pengisian' : 'Tidak Wajib');
const statusClass = computed(() => props.report.status === 'approved' ? (props.approvalInfo?.source === 'super_admin_takeover' ? 'bg-emerald-800 text-white' : 'bg-emerald-100 text-emerald-700') : props.report.status === 'waiting_approval' ? 'bg-amber-100 text-amber-700' : 'bg-blue-50 text-blue-700');
...
const calendarDays = computed(() => {
    ...
    const source = props.dailyCalendar || [];
    const byDay = Object.fromEntries(source.map((day) => [Number(day.day), day]));
    return [...Array(offset).fill(null), ...Array.from({ length: total }, (_, i) => ({ day: i + 1, date: `${props.calendarYear}-${String(props.calendarMonth).padStart(2, '0')}-${String(i + 1).padStart(2, '0')}`, status: byDay[i + 1]?.status || 'neutral', report_id: byDay[i + 1]?.report_id || null }))];
});
const calendarStatusClass = (day) => day?.status === 'approved_takeover' ? 'bg-emerald-800 text-white border-emerald-900' : day?.status === 'approved' ? 'bg-emerald-200 text-emerald-900 border-emerald-300' : day?.status === 'waiting_approval' ? 'bg-amber-200 text-amber-900 border-amber-300' : day?.status === 'in_progress' ? 'bg-blue-200 text-blue-900 border-blue-300' : day?.status === 'not_filled' ? 'bg-rose-200 text-rose-900 border-rose-300' : 'bg-slate-100 text-slate-400 border-slate-200';
...
<template>
    <InternalDashboardLayout title="Daily Report" :user="user" content-width="wide">
        <KpiEmployeeLayout>
                <KpiEmployeeHeader v-if="activePeriodId && employeeHeader?.id" :employee-id="employeeHeader.id" :period="headerPeriod" :available-periods="employeePeriods" active-tab="daily" page-title="Daily Report" :status-label="statusLabel" :status-class="statusClass" />
                <div class="mt-5 grid gap-5 xl:grid-cols-[1.05fr_0.95fr] xl:items-stretch">
                <section class="h-full rounded-xl border border-[#dce5f1] bg-white p-4">
                ...
                <section class="rounded-xl border border-[#dce5f1] bg-white p-2.5"><div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><h2 class="text-base font-bold text-[#102f66]">Kalender Daily Report</h2><p class="mt-1 text-xs text-slate-500">{{ isOwner ? 'Pantau status laporan dan pilih tanggal untuk melihat atau mengisi Daily Report.' : 'Pantau Daily Report bawahan dan pilih tanggal untuk melihat detail atau melanjutkan approval.' }}</p></div><div class="flex items-center gap-2"><strong class="min-w-[125px] text-center text-sm text-[#173467]">{{ ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'][calendarMonth - 1] }} {{ calendarYear }}</strong></div></div><div class="mt-1.5 flex flex-wrap gap-1.5 text-[10px] font-semibold"><span class="rounded-md border border-emerald-200 bg-emerald-50 px-2 py-1 text-emerald-700">● Disetujui Atasan</span><span class="rounded-md border border-emerald-700 bg-emerald-800 px-2 py-1 text-white">● Dialihkan</span><span class="rounded-md border border-amber-200 bg-amber-50 px-2 py-1 text-amber-700">● Menunggu Approval</span><span class="rounded-md border border-blue-200 bg-blue-50 px-2 py-1 text-blue-700">● Perlu Diisi</span><span class="rounded-md border border-rose-200 bg-rose-50 px-2 py-1 text-rose-700">● Tidak Diisi</span></div><div class="mt-1.5 overflow-hidden rounded-lg border border-[#dce5f1]"><div class="grid grid-cols-7 bg-[#eef4fb] text-center text-[9px] font-semibold text-[#60749a]"><span v-for="day in ['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu','Minggu']" :key="day" class="px-1 py-1">{{ day }}</span></div><div class="grid grid-cols-7 gap-0.5 p-0.5"><button v-for="(day,index) in calendarDays" :key="day ? day.date : `calendar-empty-${index}`" type="button" :disabled="!day || day.status === 'neutral'" :class="[day ? calendarStatusClass(day) : 'border-transparent bg-transparent', day?.date === targetDate ? 'ring-2 ring-[#1463e8] ring-offset-1' : '', day && day.status !== 'neutral' ? 'cursor-pointer hover:ring-2 hover:ring-blue-400' : 'cursor-default']" class="flex aspect-[2.1] min-h-[22px] items-center justify-center rounded border text-[10px] font-semibold" @click="openCalendarDay(day)">{{ day?.day || '' }}</button></div></div></section>
                </div>
                ...
                <section class="mt-5 overflow-hidden rounded-xl border border-[#dce5f1] bg-white">
                ...
                <footer v-if="report.status === 'approved'" class="mt-5 grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
                    <section class="flex items-center gap-5 rounded-xl border p-6" :class="approvalInfo?.source === 'super_admin_takeover' ? 'border-emerald-700 bg-emerald-800 text-white' : 'border-emerald-100 bg-emerald-50'"><div class="grid h-14 w-14 shrink-0 place-items-center rounded-full bg-emerald-500 text-3xl font-bold text-white">✓</div><div><h3 class="text-lg font-bold" :class="approvalInfo?.source === 'super_admin_takeover' ? 'text-white' : 'text-emerald-700'">{{ approvalInfo?.source === 'super_admin_takeover' ? 'Daily Report diambil alih dan disetujui Super Admin.' : 'Daily Report ini telah disetujui dan dikunci.' }}</h3><p class="mt-1 text-sm" :class="approvalInfo?.source === 'super_admin_takeover' ? 'text-emerald-50' : 'text-emerald-700/80'">Data tidak dapat lagi diubah setelah persetujuan final.</p></div></section>
                    <section class="rounded-xl border border-[#dce5f1] bg-white p-5"><div class="flex items-center gap-2"><span class="text-xl text-[#1463e8]">✎</span><h3 class="text-base font-bold text-[#102f66]">{{ approvalInfo?.source === 'super_admin_takeover' ? 'Persetujuan Super Admin' : 'Tanda Tangan Atasan Langsung' }}</h3></div><div class="mt-3 grid gap-4 sm:grid-cols-[1fr_1fr] sm:items-center"><div class="flex min-h-[105px] items-center justify-center rounded-lg border border-[#dce5f1] bg-white p-3"><img v-if="approvalInfo?.signature_url" :src="approvalInfo.signature_url" alt="Tanda tangan approval" class="max-h-[88px] max-w-full object-contain"><span v-else class="text-xs text-slate-400">Belum ada tanda tangan</span></div><div><p class="text-[11px] text-[#7890b3]">Disetujui oleh:</p><p class="mt-1 text-sm font-bold text-[#173467]">{{ approvalInfo?.name || '-' }}</p><p class="text-xs text-[#667b9e]">{{ approvalInfo?.position || 'Atasan Langsung' }}</p><p v-if="approvalInfo?.source_label" class="mt-2 text-xs font-semibold text-emerald-700">Sumber Persetujuan: {{ approvalInfo.source_label }}</p><div class="my-3 border-t border-[#e5ebf3]"></div><p class="text-[11px] text-[#7890b3]">Tanggal Persetujuan</p><p class="mt-1 text-xs font-semibold text-[#405d88]">{{ approvalInfo?.approved_at || '-' }} WIB</p></div></div></section>
                </footer>
                <section v-else-if="approvalInfo?.can_approve" class="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4"><p class="text-sm font-semibold text-amber-800">Daily Report menunggu persetujuan Anda.</p><button type="button" :disabled="!approvalInfo?.signature_url" class="mt-3 rounded-lg bg-[#1463e8] px-4 py-2.5 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-50" @click="approve(report.id)">Setujui Daily Report</button><p v-if="!approvalInfo?.signature_url" class="mt-2 text-xs text-rose-700">Tanda tangan akun Anda belum tersedia.</p></section>
                <section v-else-if="approvalInfo?.approval_window_expired" class="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">Batas persetujuan Atasan Langsung telah berakhir. Persetujuan hanya dapat dilakukan oleh Super Admin.</section>

                <section v-if="pendingApprovals?.length" class="mt-6 rounded-xl border border-[#dce5f1] p-4"><div class="flex flex-wrap items-center justify-between gap-3"><h2 class="font-bold text-[#102f66]">{{ user?.roleName === 'super_admin' ? 'Daily Menunggu Persetujuan' : 'Daily Bawahan Menunggu Persetujuan' }}</h2><button class="rounded-lg bg-[#1463e8] px-4 py-2 text-xs font-bold text-white disabled:opacity-50" :disabled="!selectedReportIds.length" @click="bulkApprove">Setujui Semua</button></div><div class="mt-3 divide-y"><label v-for="item in pendingApprovals" :key="item.id" class="flex items-center gap-3 py-3"><input v-model="selectedReportIds" type="checkbox" :value="item.id"><span class="flex-1 text-sm"><strong>{{ item.karyawan_nama }}</strong> · {{ item.tanggal }} · {{ item.activities_count }} kegiatan</span><button type="button" class="text-xs font-semibold text-blue-700" @click.prevent="approve(item.id)">Setujui</button></label></div></section>
        </KpiEmployeeLayout>
